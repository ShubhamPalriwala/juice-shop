  ...
  () => (req: Request, res: Response, next: NextFunction) => {
    ...
    ImageCaptchaModel.findAll({
      ...
      where: {
        UserId: UserId,
        createdAt: {
          [Op.gt]: new Date(Number(new Date()) - 300000),
        },
      },
      order: [["createdAt", "DESC"]],
    }).then((captchas: Array<ImageCaptchaModel>) => {
      if (!captchas[0] || req.body.answer === captchas[0].answer) {
        next();
      } else {
        res
          .status(401)
          .send(res.__("Wrong answer to CAPTCHA. Please try again."));
      }
    });
  };
