...
module.exports = function changePassword() {
  return (
    { query, headers, connection }: Request,
    res: Response,
    next: NextFunction
  ) => {
    ...
    if (!newPassword || newPassword === "undefined") {
      ...
      const token = headers.authorization
        ? headers.authorization.substr("Bearer=".length)
        : null;
      const loggedInUser = security.authenticatedUsers.get(token);
      if (loggedInUser) {
        if (
          currentPassword &&
          security.hash(currentPassword) !== loggedInUser.data.password
        ) {
                  ...
                  utils.solveIf(
                    ...
                    () => {
                      return (
                        user.id === 3 &&
                        !currentPassword &&
                        user.password === security.hash("slurmCl4ssic")
                      );
                    }
                  );
                  res.json({ user });
