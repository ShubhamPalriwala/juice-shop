...
async function processQuery(user: UserModel, req: Request, res: Response) {
...
function setUserName(user: UserModel, req: Request, res: Response) {
  UserModel.findByPk(user.id).then((user) => {
    if (user == null) {
      res.status(401).json({
    ...
    if (token) {
      const user: UserModel = await new Promise((resolve, reject) => {
        jwt.verify(
          ...
          (
            err: null,
            decoded: { data: UserModel | PromiseLike<UserModel> }
          ) => {
            if (err !== null) {
              res.status(401).json({
                error: "Unauthenticated user",
  ...
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    const user: UserModel = await new Promise((resolve, reject) => {
      jwt.verify(
        token,
        security.publicKey,
        (err: null, decoded: { data: UserModel | PromiseLike<UserModel> }) => {
          if (err !== null) {
            res.status(401).json({
              error: "Unauthenticated user",
            });
