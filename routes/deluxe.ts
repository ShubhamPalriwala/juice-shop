...
module.exports.upgradeToDeluxe = function upgradeToDeluxe() {
  return async (req: Request, res: Response, next: NextFunction) => {
    ...
    if (req.body.paymentMode === "wallet") {
      ...
      if (!wallet) {
        res.status(400).json({ status: "error", error: "Invalid Wallet" });
        return;
      }
      if (wallet.balance < 49) {
        res
          .status(400)
          .json({ status: "error", error: "Insuffienct funds in Wallet" });
        return;
      } else {
    ...
    if (req.body.paymentMode === "card") {
      ...
      if (
        !card ||
        card.expYear < new Date().getFullYear() ||
        (card.expYear === new Date().getFullYear() &&
          card.expMonth - 1 < new Date().getMonth())
      ) {
    ...
    user
      ...
      .then((user) => {
        utils.solveIf(challenges.freeDeluxeChallenge, () => {
          return (
            security.verify(utils.jwtFrom(req)) &&
            req.body.paymentMode !== "wallet" &&
            req.body.paymentMode !== "card"
          );
        });
...
module.exports.deluxeMembershipStatus = function deluxeMembershipStatus() {
  return (req: Request, res: Response, next: NextFunction) => {
    if (security.isCustomer(req)) {
      ...
      res
        .status(400)
        .json({ status: "error", error: "You are already a deluxe member!" });
    } else {
      res.status(400).json({
        status: "error",
