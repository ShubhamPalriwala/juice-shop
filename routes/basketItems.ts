...
module.exports.addBasketItem = function addBasketItem() {
  return (req: rawRequest, res: Response, next: NextFunction) => {
    ...
    if (
      user &&
      basketIds[0] &&
      basketIds[0] !== "undefined" &&
      user.bid != basketIds[0]
    ) {
      ...
      utils.solveIf(challenges.basketManipulateChallenge, () => {
        return (
          user &&
          basketItem.BasketId &&
...
module.exports.quantityCheckBeforeBasketItemAddition =
...
module.exports.quantityCheckBeforeBasketItemUpdate =
    ...
    return (req: Request, res: Response, next: NextFunction) => {
      BasketItemModel.findOne({ where: { id: req.params.id } })
        .then((item) => {
          ...
          utils.solveIf(challenges.basketManipulateChallenge, () => {
            return user && req.body.BasketId && user.bid != req.body.BasketId;
          }); // eslint-disable-line eqeqeq
...
async function quantityCheck(
  ...
  if (
    ...
    if (product.quantity >= quantity) {
      ...
      res.status(400).json({
        error: res.__("We are out of stock! Sorry for the inconvenience."),
      });
    }
  } else {
    res.status(400).json({
