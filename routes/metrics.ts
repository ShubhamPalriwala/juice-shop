...
exports.observeRequestMetricsMiddleware =
...
exports.observeFileUploadMetricsMiddleware =
  function observeFileUploadMetricsMiddleware() {
    return ({ file }: Request, res: Response, next: NextFunction) => {
      onFinished(res, () => {
        if (file) {
          res.statusCode < 400
            ? fileUploadsCountMetric.labels(file.mimetype).inc()
            : fileUploadErrorsMetric.labels(file.mimetype).inc();
        }
      });
      next();
    };
  ...
  const updateLoop = setInterval(() => {
    ...
    for (const { difficulty, category, solved } of Object.values(challenges)) {
      ...
      challengeStatuses.set(
        key,
        (challengeStatuses.get(key) || 0) + (solved ? 1 : 0)
      );
    ...
    UserModel.count({ where: { role: { [Op.eq]: "customer" } } }).then(
      (count) => {
        userMetrics.set({ type: "standard" }, count);
      }
    );
    UserModel.count({ where: { role: { [Op.eq]: "deluxe" } } }).then(
      (count) => {
    ...
    WalletModel.sum("balance").then((totalBalance) => {
      walletMetrics.set(totalBalance);
    });

    FeedbackModel.count().then((count) => {
      interactionsMetrics.set({ type: "feedback" }, count);
    });

    ComplaintModel.count().then((count) => {
