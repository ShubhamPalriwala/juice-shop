...
module.exports = function getLanguageList() {
  ...
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    fs.readFile(
      ...
      (err, content) => {
        if (err != null) {
          next(
            new Error(
              `Unable to retrieve en.json language file: ${err.message}`
            )
          );
        }
        enContent = JSON.parse(content);
        fs.readdir(
          "frontend/dist/frontend/assets/i18n/",
          (err, languageFiles) => {
            if (err != null) {
            ...
            languageFiles.forEach((fileName) => {
              ...
              fs.readFile(
                "frontend/dist/frontend/assets/i18n/" + fileName,
                "utf-8",
                async (err, content) => {
                  if (err != null) {
                    next(
                      new Error(
                        `Unable to retrieve ${fileName} language file: ${err.message}`
                      )
                    );
                  }
                  const fileContent = JSON.parse(content);
                  let percentage: number = Number(
                    calcPercentage(fileContent, enContent)
                  );
                  ...
                  if (locale) {
                    ...
                    if (
                      !(fileName === "en.json" || fileName === "tlh_AA.json")
                    ) {
                      languages.push(lang);
                    }
                  }
    ...
    async function calcPercentage(
      fileContent: { [x: string]: any },
      enContent: { [x: string]: any }
    ) {
