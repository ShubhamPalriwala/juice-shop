...
exports.forgedFeedbackChallenge =
...
exports.captchaBypassChallenge =
  () => (req: Request, res: Response, next: NextFunction) => {
    if (utils.notSolved(challenges.captchaBypassChallenge)) {
      if (req.app.locals.captchaReqId >= 10) {
        if (
          new Date().getTime() -
            req.app.locals.captchaBypassReqTimes[
              req.app.locals.captchaReqId - 10
            ] <=
          10000
        ) {
      ...
      req.app.locals.captchaBypassReqTimes[req.app.locals.captchaReqId - 1] =
        new Date().getTime();
...
exports.registerAdminChallenge =
  () => (req: Request, res: Response, next: NextFunction) => {
    utils.solveIf(challenges.registerAdminChallenge, () => {
      return req.body && req.body.role === security.roles.admin;
    });
    next();
  };

exports.passwordRepeatChallenge =
  () => (req: Request, res: Response, next: NextFunction) => {
    utils.solveIf(challenges.passwordRepeatChallenge, () => {
      return req.body && req.body.passwordRepeat !== req.body.password;
    });
    next();
  };

exports.accessControlChallenges =
  () =>
  ({ url }: Request, res: Response, next: NextFunction) => {
    utils.solveIf(challenges.scoreBoardChallenge, () => {
      return utils.endsWith(url, "/1px.png");
    });
    utils.solveIf(challenges.adminSectionChallenge, () => {
      return utils.endsWith(url, "/19px.png");
    });
    utils.solveIf(challenges.tokenSaleChallenge, () => {
      return utils.endsWith(url, "/56px.png");
    });
    utils.solveIf(challenges.privacyPolicyChallenge, () => {
      return utils.endsWith(url, "/81px.png");
    });
    utils.solveIf(challenges.extraLanguageChallenge, () => {
      return utils.endsWith(url, "/tlh_AA.json");
    });
    utils.solveIf(challenges.retrieveBlueprintChallenge, () => {
      return utils.endsWith(url, cache.retrieveBlueprintChallengeFile);
    });
    utils.solveIf(challenges.securityPolicyChallenge, () => {
      return utils.endsWith(url, "/security.txt");
    });
    utils.solveIf(challenges.missingEncodingChallenge, () => {
      return utils.endsWith(
        url.toLowerCase(),
        "%f0%9f%98%bc-%23zatschi-%23whoneedsfourlegs-1572600969477.jpg"
      );
    });
    utils.solveIf(challenges.accessLogDisclosureChallenge, () => {
...
exports.errorHandlingChallenge =
  () =>
  (err: Error, req: Request, { statusCode }: Response, next: NextFunction) => {
    utils.solveIf(challenges.errorHandlingChallenge, () => {
      return err && (statusCode === 200 || statusCode > 401);
    });
    next(err);
  };

exports.jwtChallenges =
  () => (req: Request, res: Response, next: NextFunction) => {
    ...
    if (
      ...
      utils.notSolved(challenges.jwtForgedChallenge)
    ) {
...
exports.serverSideChallenges =
  () => (req: Request, res: Response, next: NextFunction) => {
    if (req.query.key === "tRy_H4rd3r_n0thIng_iS_Imp0ssibl3") {
      if (
        utils.notSolved(challenges.sstiChallenge) &&
      ...
      if (
        utils.notSolved(challenges.ssrfChallenge) &&
...
function jwtChallenge(
  ...
  const token: string = utils.jwtFrom(req);
  if (token) {
    const decoded = jws.decode(token) ? jwt.decode(token) : null;
    jwt.verify(token, security.publicKey, (err: Error) => {
      if (err === null) {
        utils.solveIf(challenge, () => {
...
function hasEmail(token: { data: { email: string } }, email: string) {
  return token?.data?.email?.match(email);
}

exports.databaseRelatedChallenges =
  () => (req: Request, res: Response, next: NextFunction) => {
    ...
    for (const product of config.products) {
      if (product.urlForProductTamperingChallenge !== undefined) {
        urlForProductTamperingChallenge =
          product.urlForProductTamperingChallenge;
        break;
      }
    }
...
function knownVulnerableComponents() {
  return [
    {
      [Op.and]: [{ [Op.like]: "%sanitize-html%" }, { [Op.like]: "%1.4.2%" }],
    },
    {
      [Op.and]: [{ [Op.like]: "%express-jwt%" }, { [Op.like]: "%0.1.3%" }],
    },
  ];
}
...
function typosquattingNpmChallenge() {
  FeedbackModel.findAndCountAll({
    where: { comment: { [Op.like]: "%epilogue-js%" } },
  }).then(({ count }: { count: number }) => {
    if (count > 0) {
  ...
  ComplaintModel.findAndCountAll({
    where: { message: { [Op.like]: "%epilogue-js%" } },
  }).then(({ count }: { count: number }) => {
    if (count > 0) {
      utils.solve(challenges.typosquattingNpmChallenge);
    }
...
function typosquattingAngularChallenge() {
  FeedbackModel.findAndCountAll({
    where: { comment: { [Op.like]: "%anuglar2-qrcode%" } },
  }).then(({ count }: { count: number }) => {
    if (count > 0) {
  ...
  ComplaintModel.findAndCountAll({
    where: { message: { [Op.like]: "%anuglar2-qrcode%" } },
  }).then(({ count }: { count: number }) => {
    if (count > 0) {
      utils.solve(challenges.typosquattingAngularChallenge);
    }
...
function hiddenImageChallenge() {
  FeedbackModel.findAndCountAll({
    where: { comment: { [Op.like]: "%pickle rick%" } },
  }).then(({ count }: { count: number }) => {
    if (count > 0) {
  ...
  ComplaintModel.findAndCountAll({
    where: { message: { [Op.like]: "%pickle rick%" } },
  }).then(({ count }: { count: number }) => {
    if (count > 0) {
      utils.solve(challenges.hiddenImageChallenge);
    }
...
function supplyChainAttackChallenge() {
  FeedbackModel.findAndCountAll({
    where: { comment: { [Op.or]: eslintScopeVulnIds() } },
  }).then(({ count }: { count: number }) => {
    if (count > 0) {
  ...
  ComplaintModel.findAndCountAll({
    where: { message: { [Op.or]: eslintScopeVulnIds() } },
  }).then(({ count }: { count: number }) => {
    if (count > 0) {
      utils.solve(challenges.supplyChainAttackChallenge);
    }
...
function dangerousIngredients() {
  ...
  const dangerousProduct = config
    .get("products")
    .filter(
      (product: Product) => product.keywordsForPastebinDataLeakChallenge
    )[0];
  dangerousProduct.keywordsForPastebinDataLeakChallenge.forEach(
    (keyword: string) => {
