...
export const retrieveCodeSnippet = async (
  ...
  if (challenge) {
    if (!cache[challenge.key]) {
      ...
      if (matches[0]) {
        // TODO Currently only a single source file is supported
        const source = fs.readFileSync(path.resolve(matches[0].path), "utf8");
        const snippets = source.match(
          `[/#]{0,2} vuln-code-snippet start.*${challenge.key}([^])*vuln-code-snippet end.*${challenge.key}`
        );
        if (snippets != null) {
          let snippet = snippets[0]; // TODO Currently only a single code snippet is supported
          snippet = snippet.replace(
            /\s?[/#]{0,2} vuln-code-snippet start.*[\r\n]{0,2}/g,
            ""
          );
          snippet = snippet.replace(
            /\s?[/#]{0,2} vuln-code-snippet end.*/g,
            ""
          );
          snippet = snippet.replace(
            /.*[/#]{0,2} vuln-code-snippet hide-line[\r\n]{0,2}/g,
            ""
          );
          snippet = snippet.replace(
            /.*[/#]{0,2} vuln-code-snippet hide-start([^])*[/#]{0,2} vuln-code-snippet hide-end[\r\n]{0,2}/g,
            ""
          );
          ...
          for (let i = 0; i < lines.length; i++) {
            if (
              new RegExp(`vuln-code-snippet vuln-line.*${challenge.key}`).exec(
                lines[i]
              ) != null
            ) {
              vulnLines.push(i + 1);
              ...
              new RegExp(
                `vuln-code-snippet neutral-line.*${challenge.key}`
              ).exec(lines[i]) != null
            ) {
          ...
          snippet = snippet.replace(
            /\s?[/#]{0,2} vuln-code-snippet vuln-line.*/g,
            ""
          );
          snippet = snippet.replace(
            /\s?[/#]{0,2} vuln-code-snippet neutral-line.*/g,
            ""
          );
          cache[challenge.key] = { snippet, vulnLines, neutralLines };
        } else {
          throw new BrokenBoundary(
            "Broken code snippet boundaries for: " + challenge.key
          );
        }
      } else {
        throw new SnippetNotFound(
          "No code snippet available for: " + challenge.key
        );
      }
    }
...
exports.serveCodeSnippet =
  () =>
  async (
    req: Request<SnippetRequestBody, {}, {}>,
    res: Response,
    next: NextFunction
  ) => {
    let snippetData;
    try {
      ...
      res
        .status(setStatusCode(snippetData))
        .json({ snippet: snippetData.snippet });
    } catch (error: any) {
      //TODO
...
export const retrieveChallengesWithCodeSnippet = async () => {
  if (!cache.codingChallenges) {
    ...
    const matches = await fileSniff(SNIPPET_PATHS, match);
    cache.codingChallenges = matches
      .map((m) => m.match.trim().substr(26).trim())
      .join(" ")
      .split(" ")
      .filter((c) => c.endsWith("Challenge"));
  }
...
exports.checkVulnLines =
  () =>
  async (
    req: Request<{}, {}, VerdictRequestBody>,
    res: Response,
    next: NextFunction
  ) => {
