...
module.exports = function resetPassword() {
  return ({ body, connection }: Request, res: Response, next: NextFunction) => {
    ...
    if (!email || !answer) {
      next(
        new Error("Blocked illegal activity by " + connection.remoteAddress)
      );
      ...
      SecurityAnswerModel.findOne({
        ...
        .then((data) => {
          ...
          if (security.hmac(answer) === data.answer) {
            UserModel.findByPk(data.UserId)
              .then((user) => {
                if (!user) {
                  throw new Error("No such user found");
                }
...
function verifySecurityAnswerChallenges(user: UserModel, answer: string) {
  utils.solveIf(challenges.resetPasswordJimChallenge, () => {
    return user.id === users.jim.id && answer === "Samuel";
  });
  utils.solveIf(challenges.resetPasswordBenderChallenge, () => {
    return user.id === users.bender.id && answer === "Stop'n'Drop";
  });
  utils.solveIf(challenges.resetPasswordBjoernChallenge, () => {
    return user.id === users.bjoern.id && answer === "West-2082";
  });
  utils.solveIf(challenges.resetPasswordMortyChallenge, () => {
    return user.id === users.morty.id && answer === "5N0wb41L";
  });
  utils.solveIf(challenges.resetPasswordBjoernOwaspChallenge, () => {
    return user.id === users.bjoernOwasp.id && answer === "Zaya";
  });
  utils.solveIf(challenges.resetPasswordUvoginChallenge, () => {
    return user.id === users.uvogin.id && answer === "Silence of the Lambs";
  });
  utils.solveIf(challenges.geoStalkingMetaChallenge, () => {
    const securityAnswer = (() => {
      const memories: Array<Memory> = config.get("memories");
