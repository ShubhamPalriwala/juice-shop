...
function matchesSystemIniFile(text: string) {
  const match = text.match(
    /(; for 16-bit app support|drivers|mci|driver32|386enh|keyboard|boot|display)/gi
  );
...
function checkUploadSize({ file }: Request, res: Response, next: NextFunction) {
  utils.solveIf(challenges.uploadSizeChallenge, () => {
    if (!file) {
      return false;
    }
    ...
    if (file?.buffer && !utils.disableOnContainerEnv()) {
      ...
      try {
        ...
        utils.solveIf(challenges.xxeFileDisclosureChallenge, () => {
          return (
            matchesSystemIniFile(xmlString) || matchesEtcPasswdFile(xmlString)
          );
        });
        res.status(410);
        next(
